...
class User < ApplicationRecord
  # Módulos de Devise. Asegúrate de que no haya comas extras o saltos de línea incorrectos.
  ...
  def public_profile?
    self.profile_visibility == PROFILE_VISIBILITY[:public_profile]
  end

  def connections_only?
    self.profile_visibility == PROFILE_VISIBILITY[:connections_only]
  end

  def private_profile?
    self.profile_visibility == PROFILE_VISIBILITY[:private_profile]
  end
